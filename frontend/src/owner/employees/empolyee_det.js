...
const EmployeeDetails = () => {
  ...
  const completedTasks = employeeDuties.filter(duty => duty.employee === parseInt(employee_id) && duty.status === 'completed');
  const pendingTasks = employeeDuties.filter(duty => duty.employee === parseInt(employee_id) && duty.status === 'pending');
  const incompleteTasks = employeeDuties.filter(duty => duty.employee === parseInt(employee_id) && duty.status !== 'completed' && duty.status !== 'pending');
